...
function drawCard(activeplayer) {
    const randomNumber = Math.floor(Math.random() * (Cardgame['cards'].length));
    const currentCard = Cardgame['cards'].splice(randomNumber, 1);
...
function updateScore(currentcard, activeplayer) {
    ...
    if (currentcard == 'AC' || currentcard == 'AD' || currentcard == 'AH' || currentcard == 'AS') {
        if ((activeplayer['score'] + Cardgame['cardsmap'][currentcard][1]) <= 21) {

            activeplayer['score'] += Cardgame['cardsmap'][currentcard][1];
        } else {
            activeplayer['score'] += Cardgame['cardsmap'][currentcard][0];
        }
    } else { //For Other Cases
        activeplayer['score'] += Cardgame['cardsmap'][currentcard];
    }
}
...
function scoreboard() {
    ...
    document.querySelector('#losses').textContent = Cardgame['losses'];
...
// Deal Button
document.querySelector('#deal').addEventListener('click', Carddeal);
...
function Carddeal() {
    ...
    if (You['score'] === 0) {
        ...
        Cardgame['cards'] = ['2C', '3C', '4C', '5C', '6C', '7C', '8C', '9C', '10C', 'KC', 'QC', 'JC', 'AC', '2D', '3D', '4D', '5D', '6D', '7D', '8D', '9D', '10D', 'KD', 'QD', 'JD', 'AD', '2H', '3H', '4H', '5H', '6H', '7H', '8H', '9H', '10H', 'KH', 'QH', 'JH', 'AH', '2S', '3S', '4S', '5S', '6S', '7S', '8S', '9S', '10S', 'KS', 'QS', 'JS', 'AS'];
...
document.querySelector('#stand').addEventListener('click', Cardstand)
